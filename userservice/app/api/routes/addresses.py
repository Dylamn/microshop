...
async def create(
    ...
    new_address = address_service.create_address(session, payload)

    return new_address


@router.get("/{address_id}", response_model=AddressResource)
async def show(session: SessionDep, current_user: AuthUser, address_id: int) -> Any:
    """
    ...
    address = address_service.find_address_by_id(session, address_id)
...
async def update(
    ...
    address = address_service.update_address(session, address_id, payload, owner=current_user.id)
...
async def destroy(session: SessionDep, current_user: AuthUser, address_id: int) -> None:
    """
    ...
    address_service.delete_address(session, address_id, owner=current_user.id)
